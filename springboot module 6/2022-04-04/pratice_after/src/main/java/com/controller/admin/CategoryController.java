...
public class CategoryController {
    ...
    public String showCategories(Model model) {
        model.addAttribute("listCats", categoryService.findAll(PageRequest.of(0, 999)).getContent()); // set list thể loại
    ...
    public Object getCategory(@PathVariable("id") Long id) {
        return categoryService.findById(id); // get category theo id
    }

    @GetMapping("/add")
    public String showAddCategory(Model model) {
        model.addAttribute("listCats", categoryService.findAll(PageRequest.of(0, 999)).getContent()); // set list thể loại
        model.addAttribute("category", new Categories());  // set model category mới
        model.addAttribute("pageTitle", "Add category"); // set title hiển thị trên trang
        return "admin/category/addCategory";
    }

    @PostMapping
    public String handleAddCategory(Categories category, HttpServletRequest req, RedirectAttributes redirectAttributes) {
        Long catParent = Long.parseLong(req.getParameter("catParentId")); // get cat id  từ form để set cat parent
        category.setCatParent(catParent); // get cart from service và  set cat parent
    ...
    public String showEditCategory(@PathVariable("id") Long id, Model model) {
        Categories cat = categoryService.findById(id); // get category theo id
        model.addAttribute("pageTitle", "Edit category"); // set title hiển thị trên trang
        model.addAttribute("catParentId", cat.getCatParent() != null ? cat.getCatParent() : null); // set cat parent
    ...
    @PostMapping("/edit/{id}")
    public String handleEditCategory(@PathVariable("id") Long id, Categories category, HttpServletRequest req, RedirectAttributes redirectAttributes) {
        Categories cat = categoryService.findById(id); // get category theo id
    ...
    public String disableCategory(@PathVariable("id") Long id, @RequestParam("status") String status, RedirectAttributes redirectAttributes) {
        ...
        categoryService.updateCategory(cat); // update category vào service
    ...
    public String searchCategory(@RequestParam("q") String q, Model model) {
        List<Categories> searchCategoryList = categoryService.searchCat("%".concat(q.concat("%"))); // tìm kiếm category theo q
